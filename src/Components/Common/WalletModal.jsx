...
const WalletModal = ({ isOpen, onClose, walletData }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-gradient-to-br from-red-50 to-white rounded-lg w-full max-w-md p-6 shadow-2xl">
          ...
          {walletData.transactions.length > 0 ? (
            <div className="space-y-2">
              {walletData.transactions.map((transaction) => (
                <div key={transaction._id} className="p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="border-t border-red-100 pt-6">
          ...
          <div className="bg-white p-4 rounded-lg shadow-inner mb-4">
            <p className="text-center font-medium text-gray-700">Your Referral Code: 
              <span className="text-red-600 font-bold ml-1">G3PSR2</span>
            </p>
          </div>
          <button 
            onClick={() => navigator.clipboard.writeText('https://yourapp.com/signup?ref=G3PSR2')}
            className="w-full bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-red-700 transition-colors flex items-center justify-center"
          >
